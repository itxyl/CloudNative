...
1. 下载离线安装包，成功后在/etc/kubeasz/down/目录下有离线包
```
wget https://github.com/goharbor/harbor/releases/download/v2.3.2/harbor-offline-installer-v2.3.2.tgz
```
2. 利用ezctl(../setup/ezctl.md) 创建一个新的集群，已有集群修改同样的文件

```
#clusters/xxx/hosts 中修改如下

[harbor]
192.168.1.8 NEW_INSTALL=true

#clusters/xxx/config.yml 中修改如下

HARBOR_VER: "v2.3.2"         # 与前面下载包版本对应
HARBOR_DOMAIN: "harbor.yourdomain.com"   # 域名
HARBOR_TLS_PORT: 8443        # 端口

# 添加一个配置模板文件（默认会拷贝到harbor做启动配置文件） 

cp -p /etc/kubeasz/roles/harbor/templates/harbor-v2.1.yml.j2 /etc/kubeasz/roles/harbor/templates/harbor-v2.3.yml.j2

注释：拷贝的 harbor-v2.3.yml.j2 与上面版本号对应
```

3. 配置完成后，执行以下，完成harbor安装和docker 客户端配置
```
./ezctl setup xxx harbor
```
