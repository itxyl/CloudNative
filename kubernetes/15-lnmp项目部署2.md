...
**==重点注意: 上面的Dockerfile没有拷贝nginx.conf配置文件，需要后面用configmap来实现==**
...
![image-20220322161257315](picture/image-20220322161257315.png)
...
spec:
  ...
  - host: lnmp.discuz.com
    http:
      paths:
      - backend:
          serviceName: nginx-svc			# 与下面的service名一致
          servicePort: 80
~~~
...
~~~powershell
[root@master1 lnmp_discuz]# vim serviceaccount.yaml
apiVersion: v1
kind: ServiceAccount
metadata:
  name: harbor-sa
  namespace: lnmp-discuz
imagePullSecrets:
- name: harbor-secret   		# 名字与上面创建的secret名字一致
~~~



~~~powershell
[root@master1 lnmp_discuz]# kubectl apply -f serviceaccount.yaml
...
确认nfs服务OK(具体搭建参考前面笔记)

~~~powershell
[root@hostos ~]# cat /etc/exports
/data/nfs  *(rw,no_root_squash,sync)

[root@hostos ~]# systemctl status nfs
~~~
...
~~~powershell
[root@master1 lnmp_discuz]# vim pv.yaml
apiVersion: v1
...
~~~powershell
[root@master1 lnmp_discuz]# kubectl apply -f pv.yaml
...
~~~powershell
[root@master1 lnmp_discuz]# vim pvc.yaml
apiVersion: v1
...
~~~powershell
[root@master1 lnmp_discuz]# kubectl apply -f pvc.yaml
...
configmap的YAML，可以先准备好nginx配置文件,然后使用`kubectl create cm `创建出来，再用`kubectl get cm xxx -o yaml`得到YAML模式，修改后，结果如下:

~~~powershell
[root@master1 lnmp_discuz]# vim configmap.yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: nginxconf
...
~~~powershell
[root@master1 lnmp_discuz]# kubectl apply -f configmap.yaml
...
spec:
  ...
  selector:
    matchLabels:
      app: nginx-php			# 控制器匹配的标签: 与下面pod的标签一致
  template:
    metadata:
      labels:
        app: nginx-php			# pod的标签: 与前面的service里的匹配一致
    spec:
      serviceAccount: harbor-sa		# 服务账户，与上面创建的sa名称一致
      containers:
      - name: c1
        ...
        - name: vol-nginxconf
          ...
          subPath: nginx.conf			# 需要subpath挂载，否则nginx.conf会成为一个目录，造成启动不了

      volumes:
      - name: vol-nginxroot
        persistentVolumeClaim:
          claimName: nginx-pvc			# 与前面的pvc名称一致
      - name: vol-nginxconf
        configMap:
          name: nginxconf				# 与前面的configmap名称一致
~~~
...
![image-20220322161341056](picture/image-20220322161341056.png)


![image-20220322161358738](picture/image-20220322161358738.png)

~~~powershell
[root@hostos ~]# chmod a+w -R /data/nfs/nginxroot/
~~~

![image-20220322161416055](picture/image-20220322161416055.png)


![image-20220322161442370](picture/image-20220322161442370.png)


![image-20220322161455295](picture/image-20220322161455295.png)

![image-20220322161510635](picture/image-20220322161510635.png)


## 整合成一个YAML

将上面的所有yaml整合到一个yaml文件里，结果如下:

~~~powershell
[root@master1 lnmp_discuz]# vim lnmp-discuz.yaml
...
~~~powershell
[root@master1 lnmp_discuz]# kubectl delete -f lnmp-discuz.yaml
~~~
...
mysql因为动态供给会自动产生新的存储卷，但是原来的存储卷没有删除。所以可以把原存储卷的数据mv到新的存储卷,lnmp项目仍然直接可用

~~~powershell
[root@hostos lnmp]# ls /data/nfs/
archived-lnmp-discuz-mysql-data-mysql-0-pvc-a7036094-b21b-4410-88bc-6c000241f395  # 原存储卷
lnmp-discuz-mysql-data-mysql-0-pvc-6bc0d1fe-1543-4bd2-8461-134dab25e9b4			  # 新存储卷	
nginxroot																		  # nginx的家目录	
~~~

~~~powershell
[root@hostos lnmp]# mv /data/nfs/archived-lnmp-discuz-mysql-data-mysql-0-pvc-a7036094-b21b-4410-88bc-6c000241f395/discuz/* /data/nfs/lnmp-discuz-mysql-data-mysql-0-pvc-6bc0d1fe-1543-4bd2-8461-134dab25e9b4/discuz/
mv：是否覆盖"/data/nfs/lnmp-discuz-mysql-data-mysql-0-pvc-6bc0d1fe-1543-4bd2-8461-134dab25e9b4/discuz/db.opt"？ y
~~~
